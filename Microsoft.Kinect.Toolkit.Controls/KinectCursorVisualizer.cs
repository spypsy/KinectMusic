    ...
    public class KinectCursorVisualizer : Canvas
    {
        ...
        private void OnHandPointersUpdated(object sender, EventArgs args)
        {
            ...
            foreach (HandPointer pointer in this.pointerCursorMap.Keys)
            {
                ...
                var cursorCanvasPosition = pointer.GetPosition(this);
                ...
                //// If the cursor is not in the interactive area, show the cursor as 70% transparent
                //cursor.Opacity = pointer.IsInteractive ? 1.0 : 0.3;
                //CoordinateMapper mapper = new CoordinateMapper(kinectRegion.KinectSensor);
                //ColorImagePoint point = new ColorImagePoint();
                
                //foreach (Skeleton skel in kinectRegion.Skeletons)
                //{
                //    if (skel.TrackingState == SkeletonTrackingState.Tracked)
                //    {
                //        Joint joint = pointer.HandType == HandType.Left 
                //            ? skel.Joints[JointType.HandLeft] 
                //            : skel.Joints[JointType.HandRight];

                //        point = mapper.MapSkeletonPointToColorPoint(joint.Position, ColorImageFormat.RgbResolution640x480Fps30);
                //        point.X = (int)((point.X / 640.0) * kinectRegion.ActualWidth);
                //        point.Y = (int)((point.Y / 480.0) * kinectRegion.ActualHeight);
                //    }
                //}
